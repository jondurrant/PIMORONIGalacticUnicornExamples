/***
 * main.cpp - Automatic Brightness using the Light Sensor
	...
	for (;;) {
		...
		if((now - last) > 5000){
			...
			for (int i=0; i < 4; i++){
				...
				Point bl(0 + i * triWidth, GalacticUnicorn::HEIGHT-1 );
				Point br(triWidth + i * triWidth, GalacticUnicorn::HEIGHT-1 );
				Point tp(triWidth/2 + i*triWidth, 0);

				if (i%2 == 0){
					bl.y=0;
					br.y=0;
					tp.y= GalacticUnicorn::HEIGHT-1 ;
				}
		...
		printf("Ambient light is %u Brightness %f\n", amb,  galactic_unicorn.get_brightness());
